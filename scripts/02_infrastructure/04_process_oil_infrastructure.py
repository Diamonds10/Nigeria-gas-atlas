"""
Process the raw GOIT asset CSV into a cleaned, Nigeria-filtered output.
...
RAW_FILENAME = "goit_oil_ngl_pipelines_all_countries.csv"
OUTPUT_FILENAME = "goit_oil_ngl_pipelines_nigeria.csv"
...
def get_input_path(input_dir: Path) -> Path:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/02_infrastructure/03_download_oil_infrastructure.py first."
        )
    return path
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Convert the downloaded GOIT CSV to a cleaned, Nigeria-filtered CSV."
    )
    ...
    mask = df["countries"].astype(str).str.contains(args.country, case=False, na=False)
